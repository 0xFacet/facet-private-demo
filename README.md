...
A standalone demo of **Facet Private** — private payments on Ethereum with your existing wallet.

**[Try the live demo →](https://facet-private-demo.vercel.app/)** · **[Design overview](https://hackmd.io/@tomlehman/enshrined-privacy-pool-overview)** · **[Full spec](https://hackmd.io/@tomlehman/enshrined-privacy-pool-eip)**

## The Vision

**Facet Private** is a privacy layer for Ethereum where you use your existing wallet normally, but your balances and transfers are private. No new wallet, no new keys.

### The Privacy RPC

A Privacy RPC sits between your wallet and Ethereum:

```
Wallet → Privacy RPC → Ethereum (Sepolia)
```

Two jobs:
1. **Balances:** Decrypts your notes, reports them to your wallet
2. **Transfers:** You sign a normal send tx → Privacy RPC builds ZK proof → amounts and parties stay hidden

Anyone can run their own Privacy RPC.

---

## What This Demo Covers

This demo proves the core innovation: **ECDSA signature verification inside ZK proofs**. Your wallet signature authorizes spends; the Privacy RPC cannot forge it.
...
```
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│     Wallet      │────▶│   Privacy RPC   │────▶│     Sepolia     │
│                 │     │   (RPC Server)   │     │  (PrivacyPool)  │
└─────────────────┘     └─────────────────┘     └─────────────────┘
        │                       │                       │
...
The demo uses two signing contexts: **Sepolia** for deposits, and a **private network** (chain 13371337) for transfers and withdrawals. Final settlement is always on Sepolia.

1. **Deposit:** User sends ETH to the PrivacyPool contract on Sepolia. A "note" commitment is added to the merkle tree.

2. **Transfer:** User signs a transaction via the private network. The Privacy RPC generates a ZK proof that the signature authorizes the spend, and submits it on-chain.
...
|-----------|----------------|-------------------|
| **Privacy RPC** | Privacy (sees all your notes), Auto-registration | Spending (can't forge your signature) |
| **PrivacyPool Contract** | Proof verification | Nothing else — trustless |
| **Wallet** | Key custody | N/A |

The Privacy RPC can see your balance, but every spend requires your wallet signature verified inside the ZK circuit.
...
When you sign the "Register Viewing Key" message, the Privacy RPC derives your encryption keypair and automatically registers it on L1 via a trusted relayer. This happens transparently — you just wait a few seconds for the L1 transaction to confirm.
...
3. **Register Viewing Key** — Sign a message to derive your encryption keys. This is a one-time setup that registers you on-chain and lets the Privacy RPC decrypt notes sent to you. The Privacy RPC auto-registers you in the RecipientRegistry (takes a few seconds for the L1 tx to confirm).

4. **Make Private** — Enter an amount and click "Make Private". You'll sign a deposit transaction on Sepolia that adds ETH to your private balance.

5. **Transfer** — Enter a recipient address and amount, click "Send". The recipient must also be registered. You'll sign a transaction via the private network; proof generation takes ~60 seconds.

6. **Make Public** — Enter an amount and click "Make Public". Your private ETH returns to your public wallet.

### Tips

- Proof generation takes ~60 seconds on typical hardware
- The Privacy RPC must be running for transfers/withdrawals to work
- If the Privacy RPC restarts, click "Refresh Page" to re-register your session
...
Our approach: the Privacy RPC generates proofs server-side. Users just sign with their wallet. The tradeoff is weaker deposit privacy, but the spend unlinkability — the core privacy property — is preserved.
...
**Q: Can the Privacy RPC steal my funds?**
A: No. Every spend requires your ECDSA signature, verified inside the ZK circuit. The Privacy RPC can see your balance but cannot forge signatures. Even though the Privacy RPC auto-registers you, this only sets up your encryption key — it doesn't give anyone spending authority over your funds.

**Q: What if the Privacy RPC goes down?**
A: Your funds are safe in the on-chain contract. You'd need to run your own Privacy RPC (or wait for it to come back) to generate proofs for transfers/withdrawals.
...
**Q: Can I run my own Privacy RPC?**
A: Yes! The Privacy RPC is open source. Point it at your own Sepolia RPC and you're independent.

**Q: How is this different from Tornado Cash?**
A: Tornado uses fixed denominations and requires client-side proofs. We support variable amounts with server-side proofs and use your existing wallet.
